...
public class ResearchHandler {
	...
	private static void initThaumcraft() {
		...
		new ResearchItem("BIOMEMORPHER", "TECHNOMANCY", new AspectList().add(Aspect.EXCHANGE, 5).add(Aspect.TAINT, 5).add(Aspect.AURA, 5).add(Aspect.EARTH, 5), -2, -1, 3, new ItemStack(TMBlocks.biomeMorpher)).setPages(new ResearchPage[] { new ResearchPage("techno.research_page.BIOMEMORPHER.1"),  new ResearchPage((IArcaneRecipe)recipes.get("BiomeMorpher"))}).setParents(new String[] { "TECHNOBASICS" }).setSecondary().registerResearchItem();
