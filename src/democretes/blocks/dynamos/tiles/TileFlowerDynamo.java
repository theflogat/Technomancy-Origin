...
public class TileFlowerDynamo extends TileDynamoBase implements IManaReceiver {
		...
		for(int x = -4; x < 5; x++) {
			for(int z = -4; z < 5; z++) {
				...
				if(tile instanceof IManaPool) {
					...
					if(pool.getCurrentMana() >= 100 && this.mana <= this.maxMana - 100) {
	...
	public void renderHUD(Minecraft mc, ScaledResolution res) {
		...
		HUDHandler.drawSimpleManaHUD(color, this.mana, this.maxMana, TMBlocks.flowerDynamo.getLocalizedName(), res);
	}
