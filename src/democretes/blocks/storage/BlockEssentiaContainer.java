...
public class BlockEssentiaContainer extends BlockBase {
	...
	public BlockEssentiaContainer(int id) {
		super(id);
		this.setHardness(1F);
		this.setUnlocalizedName(Reference.MOD_PREFIX + LibNames.ESSENTIA_CONTAINER_NAME);
		...
		if (world.getBlockTileEntity(x, y, z) instanceof TileEssentiaContainer ) {
			...
			if (container.aspectFilter != null && item == null  && player.isSneaking() && side == container.facing) {
			    ...
			    if (!player.inventory.addItemStackToInventory(new ItemStack(ConfigItems.itemResource.itemID, 1, 13))) {	
			    	world.spawnEntityInWorld(new EntityItem(world, x + 0.5F + fd.offsetX / 3.0F, y + 0.5F, z + 0.5F + fd.offsetZ / 3.0F, new ItemStack(ConfigItems.itemResource.itemID, 1, 13)));
			    }			    
				return true;
			}
			//Empties Jars
			...
			if(item != null) {
				//Adds labels
				if (container.aspectFilter == null && item.getItemDamage() == 13 && item.itemID == ConfigItems.itemResource.itemID && container.aspect != null) {
				...
				if (item.itemID == ConfigItems.itemEssence.itemID && item.getItemDamage() == 1 && container.amount <= (container.maxAmount - 8)) {
						if(container.addToContainer(((IEssentiaContainerItem)(IEssentiaContainerItem)player.getHeldItem().getItem()).getAspects(player.getHeldItem()).getAspects()[0], 8) == 0) {
							item.stackSize -= 1;
							ItemStack phial = new ItemStack(ConfigItems.itemEssence.itemID, 1, 0);
				...
				if (item.itemID == ConfigItems.itemEssence.itemID && item.getItemDamage() == 0 && container.aspect != null) {
					Aspect asp = Aspect.getAspect(container.aspect.getTag());
					if (container.takeFromContainer(container.aspect, 8) == true) {
						item.stackSize -= 1;
						world.playSoundAtEntity(player, "liquid.swim", 0.25F, 1.0F);						
						ItemStack phial = new ItemStack(ConfigItems.itemEssence.itemID, 1, 1);
	...
	public void RegisterIcons(IconRegister icon) {
		this.iconJar = icon.registerIcon("technom:models/essentiaContainer");
		this.iconLiquid = icon.registerIcon("technom:models/animatedglow");
