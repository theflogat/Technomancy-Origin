...
public class TileNodeGenerator extends TileMachineBase implements IEssentiaTransport, IAspectContainer, IWandable{
	...
	void generateNode(int aurum, int taint) {
		...
		Aspect ra = BiomeHandler.getRandomBiomeTag(this.worldObj.getBiomeGenForCoords(this.xCoord, this.yCoord).biomeID, random);
		...
		ThaumcraftWorldGenerator.createNodeAt(this.getWorldObj(), xx, this.yCoord + 1, zz, type, mod, al);
		if (this.worldObj.isRemote) {
			for (int a = 0; a < 6; a++) {
				for (int b = 0; b < 6; b++) {
					...
					Thaumcraft.proxy.wispFX3(this.worldObj, xx + fx, this.yCoord + 1 + fy, zz + fz, xx + fx * 10.0F, this.yCoord + 1 + fy * 10.0F, zz + fz * 10.0F, 0.4F, b, true, 0.05F);
				}
			}
		}		
	}
	...
	void shootLightning() {
		...
		FXLightningBolt bolt = new FXLightningBolt(this.worldObj, this.xCoord + xOffset, this.yCoord + 2, this.zCoord + zOffset, xx, this.yCoord + 2, zz, this.worldObj.rand.nextLong(), 6, 0.5F, 5);
